...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const mainAudioPath = 'secretlifemedia/SecretLifeLA.mp3?versionFinal'; // Updated for cache busting
    const staticAudioPath = 'secretlifestatic/secretlife_static.mp3?versionFinal';
    
    // Log the paths for debugging
    console.log('Main audio path:', mainAudioPath); // Should now log SecretLifeLA.mp3
    ...
    audioPlayer.addEventListener('error', (e) => {
        ...
        if (audioLoadAttempts < MAX_LOAD_ATTEMPTS) {
            ...
            if (!currentPath.startsWith('./') && !currentPath.startsWith('/') && !currentPath.startsWith('http')) {
                ...
                // Instead of legacy fallbacks, fallback only to static audio
                newPath = staticAudioPath;
            }
